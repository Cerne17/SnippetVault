...
export default function InsightProgress({ points = 0 }: InsightProgressProps) {
    ...
    if (points >= 500) {
        currentRank = { label: 'Sage', icon: Wand2, threshold: 500, nextThreshold: Infinity, color: 'text-purple-600' };
    } else if (points >= 100) {
        currentRank = { label: 'Guardian', icon: Shield, threshold: 100, nextThreshold: 500, color: 'text-indigo-600' };
    } else if (points >= 10) {
        currentRank = { label: 'Expert', icon: Sparkles, threshold: 10, nextThreshold: 100, color: 'text-emerald-600' };
    }
